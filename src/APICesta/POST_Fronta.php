...
class POST_Fronta extends APICesta {
  static public function spusti(\Slim\Psr7\Request $request, \Slim\Psr7\Response $response, array $args) : \Slim\Psr7\Response {
    ...
    try {
      ...
      $LOG->error("Chyba pri vkladaní novej fronty do databázy: ".$e->getMessage());
      return self::generuj_json_odpoved(\API::odpoved_chyba(999, $e->getMessage()), $response, 500);
    }

    $fronta = Fronta::NacitajPodlaID((int)$DB->lastInsertId());
    return self::generuj_json_odpoved(\API::odpoved($fronta->DoArray()), $response);
  }
}
